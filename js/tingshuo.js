$(function(){
    ...
    function waterFlow(){
        ...
        allChild.map(function(index,element){
            ...
            if(index < rowsNum){ // 4 < 0 1 2 3 下标小于一行能放4个元素，遍历前四个正好是第一行
                // 获取当前四个元素的高度，并放入到数组中
                ...
                // 获取最小值的下标
                var minHeightOfIndex = colsHeightArr.indexOf(minHeightOfCols); //indeof找到了返回下标，找不到返回-1
                // 第二行第一个如何摆放
  ...
  function debounce(fn, delay) {
    ...
    function checkReachBottom() {
        ...
        // 获取最后一个元素距离顶部的高度
        var allChild = $(".ts-card");
        var lastBoxTop = allChild.eq(allChild.length - 1).offset().top + 240;//eq是下标
        if (lastBoxTop < scrollTop + winHeight) {

            // 加载网络数据
            $.ajax({
                type: "get",
                url: "http://localhost:5504/blueberry_sever/page_tingshuo.php",
